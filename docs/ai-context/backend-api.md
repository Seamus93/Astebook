...
- `backend/ai_agents/**`: prompt and field-agent registries; see `docs/ai-context/ai-agents.md`.
- `backend/lib/pdf.js`: PDF parsing.
- `backend/lib/docx.js`: DOCX parsing.
- `backend/lib/pdf_app.js`: PDF-app OCR integration.
- `backend/lib/ocr_input_store.js`: temporary public OCR input URL generation and safe token masking.
- `backend/lib/ocr_input_self_test.js`: optional OCR input URL reachability/content diagnostic.
- `backend/lib/document_builder.js`: DOCX/PDF output generation.
- `backend/lib/document_naming.js`: generated document display/file naming helpers.
...
- `backend/lib/mailbox_index.js`: DB/file mailbox message index abstraction and processing claims.
- `backend/lib/mailbox_browser.js`: admin mailbox list, sync/backfill and manual mailbox message processing.
- `backend/lib/mailbox_auto_processor.js`: scheduled processing for indexed mailbox rows.
- `backend/lib/mail_cache.js`: raw mailbox source cache.
- `backend/lib/imap_operation_lock.js`: serialized IMAP retry/timeout wrapper.
- `backend/lib/text.js`: shared text normalization, money/date parsing and label helpers.
...
- Detailed model/migration map lives in `docs/ai-context/database-prisma.md`.
...
- `GET /api/v1/admin/mailbox/messages`: returns indexed mailbox messages for the admin sidebar.
- `POST /api/v1/admin/mailbox/sync`: starts mailbox IMAP sync/backfill.
- `POST /api/v1/admin/mailbox/auto-process/run`: forces mailbox auto-process batch.
- `GET /api/v1/admin/mailbox/auto-process/status`: returns mailbox auto-process runtime status.
...
- AI prompt and field-agent map lives in `docs/ai-context/ai-agents.md`.
...
- Async polling for the verified PDF-app contract uses `PDF_APP_JOB_ENDPOINT=https://api.pdf-app.net/async_jobid_check` with method `GET`, raw-key `Authorization`, `Content-Type: application/json`, `Accept: application/json` and JSON body `{"job_id":"..."}`. PDF-app requires this body on `GET`; Astebook must not call `/async_jobid_check/<jobId>` or `?job_id=...`. Generic non-PDF-app job endpoints still support legacy `{jobId}` replacement or appending the job id to the path.
- Async completed status verified from PDF-app is `status:"success"` with `extraction_results[]`; Astebook treats terminal failures `failed`, `error`, `cancelled` and `canceled` as stop conditions. A `success` response with empty/whitespace OCR text is diagnosed as `ocr_empty_result` and is not cached as valid OCR.
- Diagnostics include request duration, attempts, mode and final OCR status. PDF-app HTTP/network failures use `error_type: ocr_infrastructure`. Async diagnostics also include `ocr_provider`, `ocr_mode`, `ocr_start_status`, `ocr_job_id`, `ocr_poll_attempts`, `ocr_final_status`, `ocr_text_length`, `ocr_pages`, `credits_consumed`, initial request duration, poll attempts, poll duration and total duration.
...
- Detailed OCR/PDF-app map lives in `docs/ai-context/ocr-pdf-app.md`.
...
- Detailed mailbox ingestion map lives in `docs/ai-context/mailbox-ingestion.md`.

## Scripts and Diagnostics

- `scripts/check-js.js`: syntax lint used by `npm run lint`.
- `scripts/import-processing-events-jsonl.mjs`: imports legacy processing event JSONL into Prisma tables.
- `scripts/test_pdf_app_ocr.js`: manual PDF-app OCR diagnostic against a public PDF URL.
- `scripts/diagnose_pdf_app_async_ocr.mjs`: real async PDF-app diagnostic with temporary OCR input URL and verified polling contract.
- Detailed script map lives in `docs/ai-context/scripts-diagnostics.md`.
...
- Development specification map: `docs/ai-context/dev-specs.md`.
